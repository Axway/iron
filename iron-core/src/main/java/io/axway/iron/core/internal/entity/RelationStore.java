...
public abstract class RelationStore {
    ...
    void addReverseRelationMethodHandle(ProxyFactoryBuilder<InstanceProxyContext> callHandlerBuilder, EntityStore<?> tailEntityStore) {
        ...
        checkState(reverseRelationDefinition != null, "Relation %s#%s has no reverse relation", tailEntityStore.getEntityDefinition().getEntityName(),
                   m_relationDefinition.getRelationName());
